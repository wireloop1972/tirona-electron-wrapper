...
When distributing the Tirona Electron app with the bundled Chatterbox Turbo TTS server, you'll encounter several Windows security features that users will face.
...
Note: The TTS server is now a bundled Python environment (`tts-server/python_embedded/`)
rather than a single `.exe`. The Python interpreter itself does not need individual signing,
but the installer and main Electron executable do.
...
**For the bundled TTS server (Portable Mode Python):**
- The `python_embedded/` directory is a standard CPython distribution
- Less likely to trigger false positives than PyInstaller `.exe` files
- Submit the main installer hash to antivirus vendors
...
```nsh
# Allow the bundled Python to listen on localhost for TTS
ExecWait 'netsh advfirewall firewall add rule name="Tirona TTS Server" dir=in action=allow program="$INSTDIR\resources\tts-server\python_embedded\python.exe" enable=yes'
```
...
3. **After Installation:**
   ...
   - Antivirus may flag the bundled Python environment
...
Builds:
  - Windows x64
    ...
    - resources/
      - app.asar
      - tts-server/             <-- Bundled Chatterbox Turbo server (~4-6 GB)
...
- ⚠️ Large download size (~4-6 GB with bundled TTS server + PyTorch CUDA)
...
Server: C:\Program Files\Tirona\resources\tts-server\
Models: %USERPROFILE%\.cache\huggingface\  (HF cache, ~2 GB, downloaded on first run)
Config: %APPDATA%\Tirona\
Temp:   %TEMP%\tts-audio-*.wav
```
...
2. **First Run Setup**
   ...
   - TTS model downloading from HuggingFace (~2 GB, one-time)
   - Requires NVIDIA GPU for TTS features
